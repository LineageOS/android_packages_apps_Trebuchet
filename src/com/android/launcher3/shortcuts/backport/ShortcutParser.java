...
public class ShortcutParser {
    ...
    public ShortcutParser(Context context, Resources resources, String packageName, ComponentName componentName, int resId) throws PackageManager.NameNotFoundException {
        Resources tempRes = resources;
        try {tempRes = ResourceHack.setResSdk(resources, 25);}
        catch (Exception e) {}
        mResources = tempRes;
    ...
    private void parseShortcut(XmlPullParser parser) throws IOException, XmlPullParserException {
        ...
        Drawable icon = CustomIconsProvider.getDeepShortcutIconBackport(getResourceAttribute(parser, ATTRIBUTE_SHORTCUT_ICON), mResources);
        ...
        if (isComponentExported(activity.getComponent())) {
            mShortcutsList.add(new ShortcutInfoCompat(mPackageName, id, shortLabel, longLabel, mComponentName, activity, Process.myUserHandle(), 0, true, disabledMessage, icon));
        }
    }
