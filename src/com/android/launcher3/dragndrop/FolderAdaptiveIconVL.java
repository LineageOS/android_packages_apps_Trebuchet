...
public class FolderAdaptiveIconVL extends AdaptiveIconDrawableCompat {
    private static final String TAG = "FolderAdaptiveIconVL";

    private final Drawable mBadge;
    private final Path mMask;

    private FolderAdaptiveIconVL(Drawable bg, Drawable fg, Drawable badge, Path mask) {
        super(bg, fg);
        mBadge = badge;
        mMask = mask;
    }
    ...
    public static FolderAdaptiveIconVL createFolderAdaptiveIcon(
        ...
        float sizeScaleFactor = 1 + 2 * AdaptiveIconDrawableCompat.getExtraInsetFraction();
        ...
        try {
            return new MainThreadExecutor().submit(new Callable<FolderAdaptiveIconVL>() {
                @Override
                public FolderAdaptiveIconVL call() throws Exception {
     ...
     */
    private static FolderAdaptiveIconVL createDrawableOnUiThread(FolderIcon icon,
        ...
        // Initialize preview
        float shiftFactor = AdaptiveIconDrawableCompat.getExtraInsetFraction() /
                (1 + 2 * AdaptiveIconDrawableCompat.getExtraInsetFraction());
        ...
        return new FolderAdaptiveIconVL(new ColorDrawable(bg.getBgColor()), foreground, badge, mask);
    }
